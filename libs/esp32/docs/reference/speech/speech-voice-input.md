...
When you need voice input, use this method.

```sig
speech.startVoiceInput()
```

You'd better use this method when any key is pressed. Then, release the button to get the recognition result.
...
Connect to wifi and set speech cloud service.When the A key is pressed, start voice input.
